...
def call( String propertiesFile, List<String> applicationDeploymentUnits ) {
    ...
    applicationDeploymentUnits = applicationDeploymentUnits.join('\n')

    def basicParameters = input message: 'Please Provide Parameters', ok: 'Start', parameters: [
            text( name: 'DEPLOYMENT_UNITS' defaultValue: "${applicationDeploymentUnits}", description: "Units to be updated as part of the release preparation. For those units where code references are to be updated, append the detail after the affected unit." )
